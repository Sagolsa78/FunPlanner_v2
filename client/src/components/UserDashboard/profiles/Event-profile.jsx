...
export default function EventProfile({ onBack }) {
  ...
  return (
    <div className="min-h-screen bg-[#161b22]">
      {/* Sticky Header */}
      <div className="sticky top-0 z-10 bg-slate-900 border-b border-slate-700 shadow-sm">
        ...
        <div className="bg-slate-800 rounded-lg border border-slate-700 p-6 mb-8 shadow-sm">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex-1 mb-6 lg:mb-0">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-white">{event.client?.name}</p>
            ...
            <div className="lg:ml-8 lg:flex-shrink-0">
              <div className="bg-slate-700 rounded-lg p-4 w-full lg:w-64">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-slate-800 rounded-lg border border-slate-700 p-6 shadow-sm">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-sm text-slate-300">{event.organizer?.email}</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Phone className="w-4 h-4 text-slate-400" />
                    <span className="text-sm text-slate-300">{event.organizer?.phone}</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Building className="w-4 h-4 text-slate-400" />
                    <span className="text-sm text-slate-300">{event.organizer?.company}</span>
                  </div>
                </div>
              </div>
              ...
              {/* Client */}
              <div>
                <h3 className="font-medium text-white mb-3">Client Contact</h3>
                <div className="space-y-2">
                  <div className="flex items-center space-x-3">
                    <User className="w-4 h-4 text-slate-400" />
                    <span className="text-sm text-slate-300">{event.client?.contactPerson}</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Mail className="w-4 h-4 text-slate-400" />
                    <span className="text-sm text-slate-300">{event.client?.email}</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Phone className="w-4 h-4 text-slate-400" />
                    <span className="text-sm text-slate-300">{event.client?.phone}</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Building className="w-4 h-4 text-slate-400" />
                    <span className="text-sm text-slate-300">{event.client?.name}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {event.vendors && event.vendors.length > 0 ? (
              <div className="space-y-4">
                {event.vendors.map((vendor) => (
                  <div key={vendor.id} className="border border-slate-700 rounded-lg p-4">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <span
                        className={`px-2 py-1 text-xs font-medium rounded-full ${getVendorStatusColor(vendor.status)}`}
                      >
                        {vendor.status}
                      </span>
                    </div>
            ...
            <div className="space-y-4">
              {event.timeline.map((item, index) => (
                <div key={item.id} className="flex items-start space-x-4">
                  <div className="flex-shrink-0 mt-1">{getTimelineStatusIcon(item.status)}</div>
                  <div className="flex-1 min-w-0">
              ...
              <div className="space-y-4">
                {event.tasks.map((task) => (
                  <div key={task.id} className="border border-slate-700 rounded-lg p-4">
                    ...
                    <div className="mt-2">
                      <span
                        className={`px-2 py-1 text-xs font-medium rounded-full ${task.status === "completed" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"
                          }`}
                      >
                        {task.status}
                      </span>
            ...
            {event.documents && event.documents.length > 0 ? (
              <div className="space-y-4">
                {event.documents.map((doc) => (
                  <div
                    key={doc.id}
                    className="border border-slate-700 rounded-lg p-4 hover:bg-slate-700 transition-shadow cursor-pointer group"
                  >
